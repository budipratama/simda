...
class Urusan_model extends CI_Model  {
	...
	public function get_list_tasks($list_id,$active = true){
        $this->db->select('
            ...
            urusan.no,
            urusan.fungsi,
            urusan.kode as task_id,
            tipe.tipe_nama
            ');
		$this->db->order_by('urusan.no','ASC');
        $this->db->from('urusan');
        $this->db->join('tipe', 'tipe.tipe_kode = urusan.tipe');
        $this->db->where('urusan.tipe',$list_id);
        if($active == true){
            $this->db->where('urusan.tipe_sort',1);
        } else {
            $this->db->where('urusan.tipe_sort',2);
        }
        $query = $this->db->get();
        if($query->num_rows() < 1){
            return FALSE;
