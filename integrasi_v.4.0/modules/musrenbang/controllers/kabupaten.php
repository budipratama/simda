...
class Kabupaten extends CI_Controller {
	...
	function datatable()
	{
		...
		->add_column('Actions', get_button('$1'),'kode')
	...
	public function index()
	{
		...
		$container['sidebar']['dataset']['aktive_menu'] = 10;
		$container['content']['view']					= 'musrenbang/kabupaten/view';
	...
	public function cari()
	{
		...
		redirect('musrenbang/kabupaten/hasil-pencarian/'.$tahun.'/'.$tipe.'/'.$skpd.'/'.$kecamatan.'/'.$deskel.'/'.$kegiatan);
	}
	...
	public function belanja_langsung()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			redirect('musrenbang/kabupaten/#warningEntri', 'refresh');
		} else {
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 10;
			$container['content']['view']					= 'musrenbang/kabupaten/belanja_langsung/add';
			$container['content']['dataset']['tahun']		= $this->Tahun_model->grid_all('tahun', 'tahun', 'ASC', '', '', array('status'=>'1'));
	...
	public function delete()
	{
		...
		$waktuEntri	= $this->Tahapan_model->get('selesai', array('kode' => '8'));
		$waktuSekarang = date("Y-m-d H:i:s");
		if ($waktuEntri->selesai < $waktuSekarang && $admin_log['level_kode'] != 1){
			redirect('musrenbang/kabupaten/#warningEntri', 'refresh');
		} else {
			...
			if ($anggaran->status == 1){
				...
				redirect('musrenbang/kabupaten/#successDelete', 'refresh');

			} else {

				redirect('musrenbang/kabupaten/#warningTransfer', 'refresh');
			}

			redirect('musrenbang/kabupaten', 'refresh');
		}
	}
	...
	public function detail1()
	{
		...
		if ($anggaran->tipe_kode == 1){
			...
			$container['content']['view']					= 'musrenbang/kabupaten/belanja_langsung/detail';
			...
			$container['content']['view']					= 'musrenbang/kabupaten/belanja_tidak_langsung/detail';
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'musrenbang/kabupaten/belanja_langsung/edit';
					if ($admin_log['level_kode'] == 5 || $admin_log['level_kode'] == 4){
						$skpd = $this->Skpd_model->get('skpd_kode, skpd_nama', array('skpd_kode' => $admin_log['skpd_kode']));
						$container['content']['dataset']['skpd_kode']	= $skpd->skpd_kode;
						$container['content']['dataset']['skpd_nama']	= $skpd->skpd_nama;
					...
					$container['content']['view']					= 'musrenbang/kabupaten/belanja_tidak_langsung/edit';
					$container['content']['dataset']['kode'] 		= $anggaran_btl->kode;
	...
	public function update(){
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				$apbd_sebelumnya = $this->Anggaran_model->get('1', 'anggaran_bl.apbd_kab', array('anggaran.kode'=>$kode));
				$anggaran_sebelumnya = $sisa_anggaran + $apbd_sebelumnya->apbd_kab;
				
				if ($this->input->post('apbd_kab') <= $anggaran_sebelumnya){
					...
					redirect('musrenbang/kabupaten/#successUpdate', 'refresh');
				} else {
					redirect('musrenbang/kabupaten/#warningAPBD', 'refresh');
				}
			}
	...
	public function belanja_tidak_langsung()
	{
		...
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			...
			$container['sidebar']['view']					= 'admin/sidebar';
			$container['sidebar']['dataset']['aktive_menu'] = 10;
			$container['content']['view']					= 'musrenbang/kabupaten/belanja_tidak_langsung/add';
			$container['content']['dataset']['tahun']		= $this->Tahun_model->grid_all('tahun', 'tahun', 'ASC', '', '', array('status'=>'1'));
			$container['content']['dataset']['skpd']		= $this->Skpd_model->grid_all('skpd_kode, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'SKPD'));
			$container['content']['dataset']['kecamatan']	= $this->Skpd_model->grid_all('skpd_kd, skpd_nama', 'skpd_nama', 'ASC', '', '', array('skpd_status'=>'Kecamatan'));
			$header['admin_log']							= $admin_log;
	...
	{
		$admin_log 	= $this->auth->is_login_admin();
		$waktuEntri	= $this->Tahapan_model->get('selesai', array('kode' => '9'));
		$tahapanSKPD = $this->Tahapan_skpd_model->count_all(array('skpd_kode' => $admin_log['skpd_kode'], 'tahapan_kode' => '9'));
		$waktuSekarang = date("Y-m-d H:i:s");
		if ($waktuEntri->selesai < $waktuSekarang && $tahapanSKPD == 0 && $admin_log['level_kode'] != 1){
			redirect('musrenbang/kabupaten/#warningEntri', 'refresh');
		} else {
			...
			if ($anggaran->status == 1){
				...
				if ($anggaran->tipe_kode == 1){
					...
					$container['content']['view']					= 'musrenbang/kabupaten/belanja_langsung/transfer';
					if ($admin_log['level_kode'] == 5 || $admin_log['level_kode'] == 4){
						$skpd = $this->Skpd_model->get('skpd_kode, skpd_nama', array('skpd_kode' => $admin_log['skpd_kode']));
						$container['content']['dataset']['skpd_kode']	= $skpd->skpd_kode;
						$container['content']['dataset']['skpd_nama']	= $skpd->skpd_nama;
					...
					$container['content']['view']					= 'musrenbang/kabupaten/belanja_tidak_langsung/transfer';
					$container['content']['dataset']['kode'] 		= $anggaran_btl->kode;
					$container['content']['dataset']['tahun_'] 		= $anggaran_btl->tahun;
					$container['content']['dataset']['skpd_'] 		= $anggaran_btl->pelaksana_kode;
					$container['content']['dataset']['kegiatan'] 	= $anggaran_btl->kegiatan;
					...
					$container['content']['dataset']['foto_'] 		= explode(", ", $anggaran_btl->foto);
					$container['content']['dataset']['koordinat'] 	= $anggaran_btl->koordinat;
					$container['content']['dataset']['catatan'] 	= $anggaran_btl->catatan;
				}
				$header['admin_log']								= $admin_log;
		...
		if ($type == 'bl'){
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				if ($this->input->post('apbd_kab') <= $sisa_anggaran){
					...
					redirect('musrenbang/kabupaten/#successTransfer', 'refresh');
				} else {
					redirect('musrenbang/kabupaten/#warningAPBD', 'refresh');
				}
