...
class Rekening extends CI_Controller {
	...
	public function editj($id) {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			$container['content']['view']					= 'parameter/rekening/editj';
				...
				redirect('parameter/rekening/jenis/'.$jenis->kelompok.'/#warningEdit', 'refresh');
			}
		}
	}
	...
	public function ubahj($id) {
		...
		$jenis 		= $this->Jenis_model->getOnly('akun_sort, kelompok, status', array('kode'=>$kode));	 {			
			...
			redirect('parameter/rekening/jenis/'.$jenis->kelompok, 'refresh');
			}
		}
    }
	...
	public function edito($id) {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			$container['content']['view']					= 'parameter/rekening/edito';
				...
				redirect('parameter/rekening/obyek/'.$obyek->jenis.'/#warningEdit', 'refresh');
			}
		}
	}
	...
	public function ubaho($id) {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			redirect('parameter/rekening/obyek/'.$obyek->jenis, 'refresh');
			}
		}
    }
	...
	public function editr($id) {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			$container['content']['view']					= 'parameter/rekening/editr';
				...
				redirect('parameter/rekening/rincian/'.$rincian->obyek.'/#warningEdit', 'refresh');
			}
		}
	}
	...
	public function ubahr($id) {
		$admin_log 	= $this->auth->is_login_admin(); {
			...
			redirect('parameter/rekening/rincian/'.$rincian->obyek, 'refresh');
			}
		}
    }
	...
	public function delete() {
		...
		if ($jenis->status == 1) {
			...
			$this->session->set_flashdata('success','
				...
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>
				<strong>Warning!</strong> Data "PERMANENT"</div>');
			redirect('parameter/rekening/jenis/'.$jenis->kelompok.'/#warningHapus', 'refresh');
			}
			redirect('parameter/rekening/jenis/'.$jenis->kelompok, 'refresh');
		}
	}
	...
	public function deleteo() {
		...
		if ($obyek->status == 1) {
			...
			redirect('parameter/rekening/obyek/'.$obyek->jenis.'/#warningHapus', 'refresh');
			}
			redirect('parameter/rekening/obyek/'.$obyek->jenis, 'refresh');
		}
	}
	...
	public function deleter() {
		...
		if ($rincian->status == 1) {
			...
			redirect('parameter/rekening/rincian/'.$rincian->obyek.'/#warningHapus', 'refresh');
			}
			redirect('parameter/rekening/rincian/'.$rincian->obyek, 'refresh');
		}
	}
