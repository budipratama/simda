...
class Indikator_skpd extends CI_Controller {
	...
	function datatable()
    {
        $this->datatables->select('nomor, kode, indikator_nama, skpd_nama')
		->add_column('Actions', get_buttons('$1'),'kode')
		->search_column('indikator_nama, skpd_nama')
        ->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, skpd_indikator.kode, indikator.indikator as indikator_nama, skpd.skpd_nama FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, skpd_indikator LEFT JOIN indikator ON skpd_indikator.indikator=indikator.kode LEFT JOIN skpd ON skpd_indikator.skpd=skpd.skpd_kode ORDER BY indikator.indikator ASC) indikator');
	...
	{	
		...
		$container['content']['dataset']['grid']		= $this->Indikator_skpd_model->grid_all('skpd_indikator.*, visi.visi as visi_nama, misi.misi as misi_nama, tujuan.tujuan as tujuan_nama, urusan.urusan as urusan_nama, sasaran.sasaran as sasaran_nama, indikator.indikator as indikator_nama, program.program as program_nama, skpd.skpd_nama', 'skpd.skpd_nama', 'ASC');
	...
	public function add()
	{
		...
		$container['sidebar']['view']					= 'admin/sidebar';
		$container['sidebar']['dataset']['aktive_menu'] = 20;
		$container['content']['view']					= 'pengaturan/indikator_skpd/add';
