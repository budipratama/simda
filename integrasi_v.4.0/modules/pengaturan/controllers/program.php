...
class Program extends CI_Controller {
	...
	function datatable()
    {
        $this->datatables->select('nomor, kode, program, hasil_program')
		->add_column('Actions', get_buttons('$1'),'kode')
		->search_column('program, hasil_program')
        ->from('(SELECT @ROW_NUMBER  := @ROW_NUMBER + 1 as nomor, program.kode, program.program, program.hasil_program FROM (SELECT @ROW_NUMBER := 0) ROWNUMBER, program ORDER BY program.program ASC) program');
	...
	{	
		...
		$container['content']['dataset']['grid']		= $this->Program_model->grid_all('program.kode, program.program, program.hasil_program', 'program.program', 'ASC');
