...
{
	function listbox($jenis='', $query, $name, $value, $name_value, $pilihan, $js='', $label='', $script=''){
		if (empty($jenis) || $jenis == "db"){			
			foreach ($query as $row){
				if ($pilihan == $row->$value){
					echo "<input value='".$row->$value."'>".$row->$name_value."</input>";
				} else {					
					echo "<div class='col-md-2'>" ;
					echo "<input type='text' readonly='readonly' class='form-control' name='' id='' value='".$row->$value."'> ";
					echo "</div>" ;
					echo "<div class='col-md-10'>" ;
					echo "<input type='text' readonly='readonly' class='form-control' name='' id='' value='".$row->$name_value."'>  ";
					echo "</div>" ;
				} 
			}
		} elseif ($jenis == "1d"){
			foreach ($query as $row){
				if ($pilihan == $row){
					echo "<input value='".$row."' selected>".$row."</input>";
				} else {
					echo "<div class='col-md-2'>" ;
					echo "<input type='text' readonly='readonly' class='form-control' name='' id='' value='".$row->$value." '> ";
					echo "</div>" ;
